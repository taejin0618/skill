...
class QualityChecker:
    ...
    def print_report(self):
        ...
        if self.stats['total_cases'] > 0:
            ...
            print(f"  • Lowest: {self.stats['priority_lowest']}개 ({self.stats['priority_lowest']/self.stats['total_cases']*100:.1f}%)")
    ...
    def calculate_score(self):
        ...
        score -= (1 - steps_rate) * 20  # Steps 미작성 페널티
        score -= (1 - expected_rate) * 20  # Expected Result 미작성 페널티
        score -= (1 - precond_rate) * 15  # Preconditions 미작성 페널티

        # 제안사항 페널티 (45점)
        penalty = min(len(self.suggestions) * 2, 45)
        score -= penalty
